...
export class RecipeEffects {
  ...
  fetchRecipes = this.actions$.pipe(ofType(RecipesActions.FETCH_RECIPES), switchMap(() => {
    return this.http.get<Recipe[]>('https://ng-recipe-book-46cef.firebaseio.com/recipes.json');
  }), map(recipes => {
  ...
  storeRecipes = this.actions$.pipe(ofType(RecipesActions.STORE_RECIPES), withLatestFrom(this.store.select('recipes')), switchMap(([actionData, recipesState]) => {
    return this.http.put('https://ng-recipe-book-46cef.firebaseio.com/recipes.json', recipesState.recipes);
  }));
